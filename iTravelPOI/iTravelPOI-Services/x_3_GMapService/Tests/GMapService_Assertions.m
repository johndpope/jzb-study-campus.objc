...
- (void) assert_RemoteItem:(id<GMItem>)item skipDeleteLocal:(BOOL)skipDeleteLocal {
    ...
    if(!skipDeleteLocal) {
        ...
        XCTAssertFalse(item.markedForSync, "Remote GMItem.markedForSync shouldn't be TRUE: %@", item);
    }
...
- (void) assert_LocalItem:(id<GMItem>)item {
    ...
    XCTAssertTrue(item.markedForSync, "Local GMItem.markedForSync should be TRUE: %@", item);
