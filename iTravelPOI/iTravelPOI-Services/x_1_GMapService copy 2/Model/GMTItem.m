...
// ---------------------------------------------------------------------------------------------------------------------
- (instancetype) initWithContentOfFeed:(NSDictionary *)feedDict error:(NSError * __autoreleasing *)err {
    ...
    if ( self = [self initWithName:@""] ) {
        ...
        if(nilProps.count > 0) {
            self = nil;
            if(err != nil) {
                NSError *parsingError = [self _createErrorWithDesc:[NSString stringWithFormat:@"Some properties of %@ have a nil value: %@", [self.class className], [nilProps componentsJoinedByString:@", "]]];
                *err = parsingError;
            }
        }
...
// ---------------------------------------------------------------------------------------------------------------------
- (NSError *) _createErrorWithDesc:(NSString *)desc  {
    
    NSDictionary *errInfo = [NSDictionary dictionaryWithObjectsAndKeys:desc, NSLocalizedDescriptionKey, nil];
    NSError *err = [NSError errorWithDomain:@"GMTItemErrorDomain" code:501 userInfo:errInfo];
    return err;
}
