...
//*********************************************************************************************************************
@interface PointListViewController () <SWRevealViewControllerDelegate, TagFilterViewControllerDelegate,
...
@property (nonatomic, strong) NSArray                   *PointntList;

@property (nonatomic, strong) NSArray                   *PointntOrder;
...
- (id) initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self) {
        ...
        self.PointntOrder = @[MBaseOrderByIconAsc, MBaseOrderByNameAsc];
    }
    return self;
}
...
- (void) viewWillAppear:(BOOL)animated {
    ...
    tagFilterController.filter.PointntOrder = self.PointntOrder;
    
    // La lista de puntos se carga desde el filtro activo
    self.PointntList = tagFilterController.filter.PointntList;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([[segue identifier] isEqualToString:@"PointntList_to_PointntEditor"]) {
        ...
        if(indexPath) {
            ...
            MPointnt *Pointnt = self.PointntList[[indexPath indexAtPosition:1]];
            ...
            MPointnt *copiedPointnt = (MPointnt *)[childContext objectWithID:Pointnt.objectID];

            editor.moContext = childContext;
            editor.Pointnt = copiedPointnt;
            editor.map = copiedPointnt.map;
...
- (IBAction)tbarItemOpenWith:(UIBarButtonItem *)sender {
    [OpenInActionSheetViewController showOpenInActionSheetWithController:self Pointnt:self.PointntList[0]];
...
- (void)filterHasChanged:(TagFilterViewController *)sender filter:(MComplexFilter *)filter {
    ...
    self.PointntList = filter.PointntList;
...
//---------------------------------------------------------------------------------------------------------------------
- (void) PointntEdiorSavePointnt:(PointEditorViewController *)sender {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MPointnt *itemToShow = (MPointnt *)[self.PointntList objectAtIndex:[indexPath indexAtPosition:1]];
